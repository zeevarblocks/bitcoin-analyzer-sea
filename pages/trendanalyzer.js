...
const extremeCandle = recentCandles.reduce((extreme, curr) => type === 'low' ? curr.low < extreme.low ? curr : extreme : curr.high > extreme.high ? curr : extreme );

return { price: type === 'low' ? extremeCandle.low : extremeCandle.high, time: new Date(extremeCandle.time).toLocaleString(), candle: extremeCandle, }; };

const calculateEMA = (data, period) => { const k = 2 / (period + 1); let emaArray = []; let ema = data.slice(0, period).reduce((sum, d) => sum + d.close, 0) / period;
...
const calculateRSI = (data, period = 14) => { let gains = 0, losses = 0; const rsiArray = [];

for (let i = 1; i <= period; i++) { const change = data[i].close - data[i - 1].close; if (change >= 0) gains += change; else losses -= change; }

let avgGain = gains / period; let avgLoss = losses / period; rsiArray[period] = 100 - (100 / (1 + avgGain / avgLoss));

for (let i = period + 1; i < data.length; i++) { const change = data[i].close - data[i - 1].close; const gain = change > 0 ? change : 0; const loss = change < 0 ? -change : 0;
...
const padding = new Array(period).fill(null); return [...padding, ...rsiArray.slice(period)]; };

const getPreviousEMACross = (candles) => { for (let i = candles.length - 2; i >= 1; i--) { const prev = candles[i - 1]; const curr = candles[i];
...
const detectReversal = (data) => { try { if (!data || data.length < 20) { return { valid: false, error: 'Insufficient candle data (need at least 20 candles).' }; }
