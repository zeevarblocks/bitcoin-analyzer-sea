  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-b from-gray-900 to-black min-h-screen">
      ...
      <input
        ...
        className="w-full p-2 rounded-md text-black placeholder-gray-400 mb-2"
      />
      ...
      {/* 📈 Signals Display */}
      {Object.entries(signalsToShow).map(([symbol, data]) => (
        <div
          key={symbol}
        ...
        >
          ...
          <div className="space-y-1">
            <p>
              ...
              <span className="text-blue-400">${data.currentPrice.toFixed(2)}</span>
            </p>
            <p>
              📊 <span className="font-medium text-white/70">{data.levelType?.toUpperCase()} Level:</span>{' '}
              <span className="text-yellow-300">{data.level ? data.level.toFixed(2) : 'N/A'}</span>
            </p>
            <p>
              🧭 <span className="font-medium text-white/70">Inferred {data.inferredLevelType === 'support' ? 'Support' : 'Resistance'}:</span>{' '}
              <span className="text-purple-300">{data.inferredLevel.toFixed(2)}</span>
            </p>
            <p>
              📈 <span className="font-medium text-white/70">Trend:</span>{' '}
              <span className="font-semibold text-cyan-300">{data.trend}</span>
            </p>
          </div>
          ...
          {(data.bearishContinuation || data.bullishContinuation) && (
            <div className="pt-4 border-t border-white/10 space-y-2">
              ...
              {data.bearishContinuation && (
                <p className="text-red-400">🔻 Bearish Continuation: <span className="font-semibold">Yes</span></p>
              )}
              {data.bullishContinuation && (
                <p className="text-green-400">🔺 Bullish Continuation: <span className="font-semibold">Yes</span></p>
              )}
            </div>
          )}
          ...
          {(data.ema14Bounce || data.ema70Bounce || data.touchedEMA70Today) && (
            <div className="pt-4 border-t border-white/10 space-y-2">
              ...
              {data.touchedEMA70Today && (
                <p className="text-blue-300">🧲 EMA70 Tested Today: <span className="font-semibold">Yes</span></p>
              )}
            </div>
          )}
          ...
          {(data.divergenceFromLevel || data.divergence || data.nearOrAtEMA70Divergence) && (
            <div className="pt-4 border-t border-white/10 space-y-2">
              ...
              {data.divergence && (
                <p className="text-orange-400">📉 RSI High/Low Divergence: <span className="font-semibold">Yes</span></p>
              )}
