...
async function getSignalData(symbol: string): Promise<SignalData> {
...
export default function SignalChecker() {
  const defaultSymbols = ['BTC-USDT', 'ETH-USDT', 'SOL-USDT', 'PI-USDT', 'CORE-USDT'];
  ...
  const fetchSignals = async (symbols: string[]) => {
    ...
    setSignals(prev => ({ ...prev, ...results }));
    setLoading(false);
  };

  const handleAddSymbol = () => {
    if (!customSymbol.trim()) return;
    fetchSignals([customSymbol.trim().toUpperCase()]);
    setCustomSymbol('');
  };

  React.useEffect(() => {
    fetchSignals(defaultSymbols);
  }, []);
    ...
    <div className="p-4 space-y-6">
      <div className="flex gap-2">
        <input
          ...
          placeholder="Enter futures pair e.g. XRP-USDT"
          className="p-2 border rounded bg-black/40 text-white"
        />
        <button onClick={handleAddSymbol} className="bg-blue-500 text-white px-4 py-2 rounded">
          Add
        </button>
      </div>
