  ...
  <div className="p-4">
    ...
    {recentATHData && (
      <div className="mt-4 bg-gray-100 p-4 rounded shadow">
        ...
        <p><strong>Recent ATH:</strong> {recentATHData.price}</p>
        <p><strong>EMA70 at ATH:</strong> {recentATHData.emaAtATH.toFixed(2)}</p>
        <p><strong>Gap to EMA70:</strong> {recentATHData.gap.toFixed(2)}%</p>
        <p><strong>Market Zone:</strong> {zone}</p>
      </div>
    )}

    {/* Debugging info for previous ATH and ATL (optional) */}
    {previousATH && (
      <div className="mt-4 text-sm text-gray-600">
        <p>Previous ATH: {previousATH.price} on {previousATH.date}</p>
      </div>
    )}
    {previousATL && (
      <div className="mt-2 text-sm text-gray-600">
        <p>Previous ATL: {previousATL.price} on {previousATL.date}</p>
      </div>
    )}
  </div>
);
